...
public class CreateApplicationVersionMojo extends AbstractBeanstalkMojo {
  ...
  protected Object executeInternal() throws MojoExecutionException {
    if (skipExisting) {
      if (versionLabelExists()) {
        getLog().info("VersionLabel " + versionLabel + " already exists. Skipping creation of new application-version");
  ...
  private boolean versionLabelExists() {
     ...
     */
    DescribeApplicationVersionsRequest davRequest =
        new DescribeApplicationVersionsRequest().withApplicationName(applicationName).withVersionLabels(versionLabel);
