...
public class ReplaceEnvironmentMojo extends CreateEnvironmentMojo {
  ...
  protected Object executeInternal() throws Exception {
     ...
     * // I really meant it.
     */
    String newEnvironmentName = getNewEnvironmentName(StringUtils.defaultString(this.environmentName, curEnv.getEnvironmentName()));
     ...
     */
    {
      ...
      for (int i = 1; i <= maxAttempts; i++) {
        try {
          swapEnvironmentCNames(newEnvDesc.getEnvironmentId(), curEnv.getEnvironmentId(), cnamePrefix, newEnvDesc);
          swapped = true;
          break;
        } catch (Throwable exc) {
          ...
          getLog().warn(format("Attempt #%d/%d failed. Sleeping and retrying. Reason: %s (type: %s)", i, maxAttempts, exc.getMessage(), exc.getClass()));
  ...
  private void copyOptionSettings(EnvironmentDescription curEnv) throws Exception {
    ...
    DescribeConfigurationSettingsResult configSettings =
        getService()
    ...
    List<ConfigurationOptionSetting> newOptionSettings =
        new ArrayList<ConfigurationOptionSetting>(configSettings.getConfigurationSettings().get(0).getOptionSettings());
    ...
    while (listIterator.hasNext()) {
      ...
      if (bInvalid) {
        getLog()
            .info(
                format(
                    "Excluding Option Setting: %s:%s['%s']",
                    curOptionSetting.getNamespace(),
                    curOptionSetting.getOptionName(),
                    CredentialsUtil.redact(curOptionSetting.getValue())));
        listIterator.remove();
      } else {
        getLog()
            .info(
                format(
                    "Including Option Setting: %s:%s['%s']",
                    curOptionSetting.getNamespace(),
                    curOptionSetting.getOptionName(),
                    CredentialsUtil.redact(curOptionSetting.getValue())));
      }
    ...
    if (null != __secGroups) {
      ...
      if (!StringUtils.isBlank(securityGroups)) {
        ConfigurationOptionSetting newOptionSetting = new ConfigurationOptionSetting("aws:autoscaling:launchconfiguration", "SecurityGroups", securityGroups);
        newOptionSettings.add(newOptionSetting);
        getLog()
            .info(
                format(
                    "Including Option Setting: %s:%s['%s']", newOptionSetting.getNamespace(), newOptionSetting.getOptionName(), newOptionSetting.getValue()));
      }
    }
   ...
   */
  protected void swapEnvironmentCNames(String newEnvironmentId, String curEnvironmentId, String cnamePrefix, EnvironmentDescription newEnv)
      throws AbstractMojoExecutionException {
    getLog().info("Swapping environment cnames " + newEnvironmentId + " and " + curEnvironmentId);

    {
      SwapCNamesContext context =
          SwapCNamesContextBuilder.swapCNamesContext() //
    ...
    if (null != domains) {
      ...
      if (!domainsToUse.isEmpty()) {
        ...
        final BindDomainsContext ctx = new BindDomainsContextBuilder().withCurEnv(newEnv).withDomains(domainsToUse).build();
  ...
  protected void terminateEnvironment(String environmentId) throws AbstractMojoExecutionException {
    ...
    for (int i = 1; i <= maxAttempts; i++) {
      ...
      try {
        TerminateEnvironmentContext terminatecontext =
            new TerminateEnvironmentContextBuilder().withEnvironmentId(environmentId).withTerminateResources(true).build();
  ...
  protected EnvironmentDescription waitForEnvironment(String environmentId) throws AbstractMojoExecutionException {
    ...
    WaitForEnvironmentContext context =
        new WaitForEnvironmentContextBuilder() //
            .withApplicationName(applicationName) //
            .withStatusToWaitFor("Ready")
            .withEnvironmentRef(environmentId) //
   ...
   * @return EnvironmentDescription in Ready state
   */
  protected EnvironmentDescription waitForGreenEnvironment(String environmentId) throws AbstractMojoExecutionException {
    getLog().info("Waiting for environmentId " + environmentId + " to become 'Green'");
  ...
  protected String getCNamePrefixToCreate() {
    ...
    while (hasEnvironmentFor(applicationName, cnamePrefixToReturn) || isNamedEnvironmentUnavailable(cnamePrefixToReturn)) {
